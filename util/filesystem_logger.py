...
class FilesystemLogger(LightningLoggerBase):
    ...
    def experiment(self):
        if self._experiment is None:
            ...
            experiment_dir = Path(f"{self.experiment_config['base_dir']}/runs", self.experiment_config["experiment"])
            experiment_dir.mkdir(exist_ok=True, parents=True)

            src_folders = ['config', 'criteria', 'data', 'dataset', 'model', 'tests', 'trainer', 'util']
            sources = []
            for src in src_folders:
                sources.extend(list(Path(".").glob(f'{src}/**/*')))

            files_to_copy = [x for x in sources if x.suffix in [".py", ".pyx", ".txt", ".so", ".pyd", ".h", ".cu", ".c",
                                                                '.cpp', ".html"] and x.parts[0] != "runs" and
                             x.parts[0] != "wandb"]
