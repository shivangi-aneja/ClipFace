...
class StyleGANOptimizer(pl.LightningModule, ABC):
    ...
    def configure_optimizers(self):
        ...
        code_opt = torch.optim.Adam([{'params': trainable_texure_params, 'lr': self.config.lr_tex},
                                     {'params': list(self.geometry_mapper.parameters()), 'lr': self.config.lr_exp}],
                                    lr=self.config.lr_exp)
        return code_opt
    ...
    def forward(self, batch):
        ...
        geo_offset = self.geometry_mapper(w.mean(dim=1))
        expression_params = batch['flame_exp'] + geo_offset
        init_vertices = self.flame(shape_params=batch['flame_shape'], expression_params=batch['flame_exp'],
                                   pose_params=batch['flame_pose'])
        pred_vertices = self.flame(shape_params=batch['flame_shape'], expression_params=expression_params,
                                   pose_params=batch['flame_pose'])
        init_img = self.render(init_texture, init_vertices, batch)
        pred_img = self.render(predicted_texture, pred_vertices, batch)
        return {'tex': predicted_texture, 'init_img': init_img, 'pred_img': pred_img, 'init_vertices': init_vertices, 'pred_vertices': pred_vertices,
    ...
    def training_step(self, batch, batch_idx):
        ...
        exp_reg = self.config.lambda_exp*torch.linalg.norm(results['expression_params'])
        total_loss = clip_loss + exp_reg
        self.log(f"train/clip_loss", clip_loss.item(), on_step=True, on_epoch=False, prog_bar=True, logger=True)
        self.log(f"train/exp_reg", exp_reg.item(), on_step=True, on_epoch=False, prog_bar=True, logger=True)
        self.log(f"train/total_loss", total_loss.item(), on_step=True, on_epoch=False, prog_bar=True, logger=True)
        self.manual_backward(total_loss)
        step(code_opt, self.texture_mapper_list + [self.geometry_mapper])
    ...
    def render(self, texture_image, flame_verts, batch):
        ...
        for i in range(self.config.batch_size):
            ...
            h, w = [custom_scale_factor_image * self.config.image_size, custom_scale_factor_image * self.config.image_size]
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log(f"val/exp_reg", exp_reg.item(), on_step=True, on_epoch=False, prog_bar=True, logger=True)
        self.log(f"val/total_loss", total_loss.item(), on_step=True, on_epoch=False, prog_bar=True, logger=True)
        torch.cuda.empty_cache()
    ...
    def export_images(self, odir_grid, odir_samples, odir_texture, odir_ckpts):
        ...
        save_image(vis_generated_images, odir_grid / f"{self.global_step:06d}.png", nrow=4, value_range=(-1, 1),
                   normalize=True)
        mapper_state_dict = {}
        for level, mapper in enumerate(self.texture_mapper_list):
            mapper_state_dict[f"level_{level}"] = mapper.state_dict()
        mapper_state_dict["geo"] = self.geometry_mapper.state_dict()
    ...
    def run_post_device_setup(self):
        ...
        self.geometry_mapper.to(self.device)
